...
def load_data_set(file_name):
    ...
    :param file_name:
        文件名
    :return:
        必须是 np.array 或者 np.matrix 不然后面没有，shape
    """
...
def stump_classify(data_mat, dimen, thresh_val, thresh_ineq):
    """
    (将数据集，按照 feature 列的 value 进行二分法切分比较来赋值分类)
    :param data_mat:
         Matrix 数据集
    :param dimen:
         特征的一个列
    ...
    #   data_mat[:,dimen] 表示数据集中第 dimen 列的所有值
    #   thresh_ineq == 'lt' 表示修改左边的值，gt 表示修改右边的值
    #   (这里其实我建议理解为转换左右边，就是一棵树的左右孩子，可能有点问题)
    if thresh_ineq == 'lt':
...
def build_stump(data_str, class_labels, D):
    """
    ...
    for i in range(n):
        ...
        for j in range(-1, int(num_steps) + 1):
            for inequal in ['lt', 'gt']:
                ...
                weighted_err    表示整体结果的错误率
                best_class_est  预测的最优结果（与class_labels 对应）
                '''
    ...
    #   best_stump 表示分类器的结果，在第几个列上，用大于/小于比较，阀值是多少（单个弱分类器）
...
def ada_boost_train_ds(data_arr, class_labels, num_it=40):
    ...
    :param num_it:
         迭代次数
    :return:
         weak_class_arr  弱分类器的集合
         agg_class_est   预测的分类结果值
    """
    weak_class_arr = []
    m = np.shape(data_arr)[0]
    #    初始化 D，设置每个特征的权重值，平均分为 m 份
    ...
    for i in range(num_it):
        ...
        #   分类准确：乘积为 1，不会影响结果，-1主要是下面求 e 的 -alpha 次方
        #   分类错误：乘积为 -1，结果会收影响，所以也乘以 -1
        expon = np.multiply(-1 * alpha * np.mat(class_labels).T, class_est)
        #   判断正确的，就乘以 -1，否则就乘以 1，为什么？书上的公式
        print('( -1 取反)预测值 expon = ', expon.T)
        #   计算 e 的 expon 次方，然后计算得到一个综合的概率的值
        #   结果发现：判断错误的样本，D 对于的样本权重值会变大
        #   multiply 是对应项相乘
        ...
        #   sign 判断正为 1，0为0，负为 -1，通过最终加和的权重值，判断符号
        #   结果为：错误的样本标签集合，因为是 != ，那么结果就是 0 正，1 负
...
def plot_roc(pred_strengths, class_labels):
    """
    打印 ROC 曲线，并计算 AUC 的面积大小
    :param pred_strengths:
        最终预测结果的权重值
    :param class_labels:
        原始数据的分类结果集
    :return:
...
def test():
    ...
    data_mat, class_labels = load_data_set(
        '/Users/wangjf/WorkSpace/MachineLearning/input/7.AdaBoost/horseColicTraining2.txt')
    print(data_mat.shape, len(class_labels))
    weak_class_arr, agg_class_est = ada_boost_train_ds(data_mat, class_labels, 1000)
    print(weak_class_arr, '\n-----\n', agg_class_est.T)
    plot_roc(agg_class_est.T, class_labels)
    data_arr_test, label_arr_test = load_data_set(
        "/Users/wangjf/WorkSpace/MachineLearning/input/7.AdaBoost/horseColicTest2.txt")
