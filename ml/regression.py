...
def loadDataSet(fileName):
    ...
    for line in fr.readlines():
        #   读取每一行
        lineArr = []
        #   删除一行中以 tab 分割的数据前后的空白符号
        curLine = line.strip().split('\t')
        #   i 从 0 到 2，不包括 2
        for i in range(numFeat):
            #   将数据添加到 lineArr List 中，每一行数据测试数据
            lineArr.append(float(curLine[i]))
            #   将数据的输入数据部分存储到 dataMat 的 List 中
        dataMat.append(lineArr)
        #   将每一行的最后一个数据，即类别，或者叫目标变量存储到 labelMat List 中
...
def standRegres(xArr, yArr):
    ...
    线性回归
    :param xArr:
        输入端样本数据，包含每个样本数据的 feature
    :param yArr:
        对应于输入数据的类别标签，也就是每个彦斌对应的目标变量
    :return:
        ws  回归系数
    """
    #   mat() 函数将 xArr,yArr 转换为矩阵 mat().T 代表的是对矩阵进行转置操作
    xMat = mat(xArr)
    yMat = mat(yArr).T
    #   矩阵乘法的条件是左矩阵的列数等于右矩阵的行数
    xTx = xMat.T * xMat
    #   因为要用到 xTx 的逆矩阵，所以事先需要确定计算得到的 xTx 是否可逆，条件是矩阵的行列式不为 0
    #   linalg.det() 函数是用来求得矩阵的行列式的，如果矩阵的行列式为 0，则这个矩阵是不可逆的，就无法进行接下来的运算
...
def lwlr(testPoint, xArr, yArr, k=1.0):
    ...
    :return:
        testPoint * ws  数据点与具有权重
    :Notes:
        这其中会用到计算权重的公式，w = e^((x^((i))-x) / -2k^2)
        理解：x 为某个预测点，x^((i)) 为样本点，样本点距离预测点越近，贡献的误差越大（权重越大），越远则贡献的误差越小（权重越小）。
        关于预测点的选取，在我的代码中取的是样本点。其中 k 是带宽参数，控制 w （钟形函数）的宽窄程度，类似于高斯函数的标准差。
        算法思路：假设预测点取样本点中的第 i 个样本点（共 m 个样本点），遍历 1 到 m 个样本点（含第 i 个），算出每一个样本点与预测点的距离，也就可以计算出每个
        样本贡献误差的权重，可以看出 w 是一个有 m 个元素的向量（写成对角阵形式）
    """
    #   mat() 函数是将 array 转换为矩阵的函数，mat().T 是转换为矩阵之后，再进行转置操作
    xMat = mat(xArr)
    yMat = mat(yArr).T
    #   获得 xMat 矩阵的行数
    m = shape(xMat)[0]
    #   eye() 返回一个对角线元素为 1，其他元素为 0 的二维数组，创建权重矩阵 weights，该矩阵为每个样本点初始化了一个权重
    weights = mat(eye((m)))
    for j in range(m):
        #   testPoint 的形式是一个行向量的形式
        #   计算 testPoint 与输入样本点之间的距离，然后下面计算出每个样本贡献误差的权重
...
def lwlrTestPlot(xArr, yArr, k=1.0):
    ...
    :param xArr:
        样本的特征数据，即 feature
    :param yArr:
        每个样本对应的类别标签，即目标向量，实际值
    :param k:
        控制核函数的衰减速度的有关参数，这里设定的是常量值 1
    :return:
        yHat    样本点的估计值
        xCopy   xArr 的复制
    """
...
def ridgeRegress(xMat, yMat, lam=0.2):
    ...
    如果数据的特征比样本点还多，就不能再使用上面介绍的线性回归和局部进行回归了，因为计算 (xTx)^(-1) 会出现错误。
    如果特征比样本点还多(n > m)，也就是说，输入数据的矩阵 x 不是满秩矩阵。非满秩矩阵在求逆时会出现问题。
    为了解决这个问题，我们下面讲一下:岭回归，这是我们要讲的第一种缩减方法
    :param xMat:
        样本的特征数据，即 feature
    :param yMat:
        每个样本对应的类别标签，即目标向量，实际值
    ...
    """
    xTx = xMat.T * xMat
    #   岭回归就是在矩阵 xTx 上加一个 ℷI 从而使得矩阵非奇异，进而能对 xTx + ℷI 求逆
    denom = xTx + eye(shape(xMat)[1]) * lam
    #   检查行列式是否为零，即矩阵是否可逆，行列式为 0 的话就不可逆，不为 0 的话就是可逆
...
def ridgeTest(xArr, yArr):
    ...
    #   可以在 30 个不同的 lambda 下调用 ridgeRegress() 函数
    numTestPts = 30
    #    创建 30 * m 的全部数据为 0 的矩阵
...
def stageWise(xArr, yArr, eps=0.01, numIt=100):
    ...
    yMat = mat(yArr).T
    yMeans = mean(yMat, 0)
    yMat = yMat - yMeans  # 也可以规则化 ys 但会得到更小的 coef
...
def searchForSet(retX, retY, setNum, yr, numPce, origPrc):
    ...
    searchUrl = 'https://www.googleapis.com/shopping/search/v1/public/products?key=%s&country=CN&q=lego+%d&alt=json' \
                % (myApiStr, setNum)
...
def setDataCollect(retX, retY):
    scrapePage(retX, retY, '', 2006, 800, 49.99)
    scrapePage(retX, retY, '', 2002, 3096, 269.99)
    scrapePage(retX, retY, '', 2007, 5195, 499.99)
    scrapePage(retX, retY, '', 2007, 3428, 199.99)
    scrapePage(retX, retY, '', 2008, 5922, 299.99)
    scrapePage(retX, retY, '', 2009, 3263, 249.99)


#   交叉验证测试岭回归
def crossValidation(xArr, yArr, numVal=10):
    #   获得数据点个数，xArr,yArr具有相同长度
    m = len(yArr)
    indexList = range(m)
...
def regression1():
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)  # add_support(349) 函数的参数的意思是，将画布分成 3 列 4 行图像画在从左到右从上到下第 9 块
    ax.scatter([xMat[:, 1].flatten()], [yMat.T[:, 0].flatten().A[0]])  # scatter 的 x 是 xMat 中的第二列，y 是 yMat 的第一列
    xCopy = xMat.copy()
    xCopy.sort(0)
    yHat = xCopy * ws
