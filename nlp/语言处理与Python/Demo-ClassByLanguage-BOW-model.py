...
"""
    Time    : 2018/7/20 下午3:23
    Author  : wangjf
    File    : Demo-ClassByLanguage-BOW-model.py
...
#   word_to_ix 将词汇表中的每个单词映射到一个唯一的函数，这将成为单词向量袋中的索引
word_to_ix = {}
...
class BowClassifier(nn.Module):
    def __init__(self, num_labels, vocab_size):
        #   调用 nn.Module 的 init 函数。不要被语法混淆，只是在 nn.Module 中做
        super(BowClassifier, self).__init__()

        #   定义您将需要的参数
        #   在这种情况下，我们需要 A 和 b，仿射映射的参数
        #   Torch 定义了 nn.Linear()，它提供了仿射图
        #   确保你明白为什么输入维度是 vocab_size，输出是 num_labels
        self.linear = nn.Linear(vocab_size, num_labels)

        #   NOTE! 非线性日志 softmax 没有参数！所有我们不需要为此担心

    def forward(self, bow_vec):
        #   通过线性层传递输入
        #   然后通过 log_softmax 传递，（使用对数形式的 softmax 函数）
        #   许多非线性和其他功能在 torch.nn.functional 中
        #   SVM 只选自己喜欢的神，Softmax 把所有的备胎全部拉出来评分，最后还归一化一下
...
def make_bow_vector(sentence, word_to_ix):
    ...
    #   返回一个有相同数据但大小不同的新的 tensor。（-1 表示其他维度）
    """
    #   -1 表示自己不确定，让系统来计算
    y = x.view(4,2)
    print(y)
...
for epoch in range(100):
    for instance, label in data:
        ...
        #   步骤2
        #   制作我们的 BOW 矢量，并且我们还必须将目标包装在一个
        #   作为整数变量
        #   例如：如果目标是西班牙语，那么我们包装整数 0
        #   然后，损失函数知道日志概率的第 0 个元素是对应西班牙语的日志概率
        bow_vec = autograd.Variable(make_bow_vector(instance, word_to_ix))
        ...
        #   步骤4
        #   通过调用 optimizer.step() 来计算损失，梯度和更新参数
